...
const done = (id: string, iso: string) =>
  new Visit({ id, fieldId: 'f1', status: 'DONE', visitedAt: D(iso), createdAt: D(iso) });
const pending = (id: string, plannedIso: string) =>
  new Visit({ id, fieldId: 'f1', status: 'PENDING', plannedFor: D(plannedIso), reminderLeadDays: 3, createdAt: D(plannedIso) });

describe('GetFieldHistory', () => {
  it('resolves the field label and returns visits newest-first', async () => {
    const { uc, visits } = setup();
    await visits.save(done('v1', '2026-07-20T10:00:00Z'));
    await visits.save(done('v2', '2026-07-27T10:00:00Z'));
  ...
  it('returns a single list mixing pending and done sorted by effective date', async () => {
    const { uc, visits } = setup();
    await visits.save(done('v1', '2026-07-20T10:00:00Z'));
    await visits.save(pending('p1', '2026-08-10T00:00:00Z'));
    await visits.save(done('v2', '2026-07-27T10:00:00Z'));
    const view = await uc.execute('f1');
    expect(view?.visits.map((v) => v.id)).toEqual(['p1', 'v2', 'v1']);
  });
  ...
  it('includes cancelled visits in the history', async () => {
    ...
    await visits.save(new Visit({
      ...
      visitedAt: D('2026-07-20T10:00:00Z'), createdAt: D('2026-07-20T10:00:00Z'),
      cancelledAt: D('2026-07-21T10:00:00Z'),
    }));
    const view = await uc.execute('f1');
